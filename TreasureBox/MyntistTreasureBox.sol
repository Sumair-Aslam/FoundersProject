...
contract MyntistTreasureBox {
    ...
    uint256 private constant MIN_ETH_DEPOSIT = 0.00005 ether;
    uint256 private constant MAX_ETH_DEPOSIT = 100 ether;
    uint256 private constant MIN_TOKEN_DEPOSIT = 1 ether;
    uint256 private constant MAX_TOKENS_DEPOSIT = 20000 ether;
    uint256 public constant BASE_AMOUNT = 20000; 
    ...
    /**
     * @dev Calculates the total interest of user and returns the reward based on deposit and duration and suppports bigger/longer pays for better payouts.
     * @param _claimDate The future date when the box can be claimed.
     * @param _depositAmount The amount of ETH deposited.
     * @return uint256 representing the calculated reward.
     */
    function calculateReward(uint256 _claimDate, uint256 _depositAmount) private view returns (uint256) {
        uint256 duration = (_claimDate - block.timestamp) / 1 days;
        uint256 reward = (_depositAmount * BASE_AMOUNT * (100 + (_depositAmount / SCALE) ** 2)) / 100;
        return (duration * reward) / MIN_DURATION;
    }  
     ...
     */
    function createTreasureBoxWithEth(uint256 _claimDate, NftInfo[] calldata _nftInfos) public payable {
        ...
        if (!isMainMyntLaunched) {
            ...
            require(rewardTokens <= FMYNT_INSTANCE.getTreasureBoxAssests(), "Reward exceeds from treasure box available supply");
        } else{
    ...
    */
    function createTreasureBoxWithTokens(uint256 _claimDate, NftInfo[] calldata _nftInfos, uint256 _tokenAmount) public {
    ...
    function calculateRewardEthToTokens(uint256 _claimDate, NftInfo[] calldata _nftInfos, uint256 _depositAmountinEth) public view returns (uint256) {
        require(_depositAmountinEth >= MIN_ETH_DEPOSIT, "Deposit must be atleast 0.00005 ether");
        require(_depositAmountinEth <= MAX_ETH_DEPOSIT, "Deposit out of range");
        ...
        require(_nftInfos.length >= MIN_NFT, "Atleast 1 NFT required");

        uint256 rewardTokens = calculateReward(_claimDate, _depositAmountinEth);
    ...
    function calculateRewardTokenToTokens(uint256 _claimDate, NftInfo[] calldata _nftInfos, uint256 _depositAmountInTokens) public view returns (uint256) {
        require(_depositAmountInTokens >= MIN_TOKEN_DEPOSIT, "Deposit must be atleast 1 ether");
        require(_depositAmountInTokens <= MAX_TOKENS_DEPOSIT, "Deposit out of range");
        require(_claimDate > block.timestamp, "Claim date must be in the future");
        require(_claimDate <= block.timestamp + MAX_CREATION_DAYS, "Claim date higher than maximum");
        require(_claimDate >= block.timestamp + MIN_CREATION_DAYS, "Claim date less than minimum");
        require(_nftInfos.length >= MIN_NFT, "Atleast 1 NFT required"); 
        
        uint256 rewardTokens = _depositAmountInTokens;
    ...
    function allocateNFTRewards(uint256 boxId, NftInfo[] calldata _nftInfos, uint256 _totalReward) private {
        ...
        for (uint256 i = 0; i < _nftInfos.length; ++i) {
            ...
            require(_nftInfos[i].nftValue > 0 && _nftInfos[i].nftValue <= 9, "NFT value must be greater than 0 and less than or equal to 9");
    ...
    */
    function setMainMyntContract(address _mainMyntContractAddress) public onlyOwner {
     ...
     */
    function claimTreasureBox(uint256 _boxId, uint256 _nftId) public {
        ...
        if (isMainMyntLaunched) {
            require(GLOBALS_INSTANCE.treasureBoxAddress() != address(0), "Treasure box address is not set in MYNT");
            GLOBALS_INSTANCE.transferReward(msg.sender, rewardAmount, GLOBALS_INSTANCE.treasureBoxAddress());
        } else {
            require(FMYNT_INSTANCE.TREASURE_BOX() != address(0), "Treasure box address is not set in FMYNT");
     ...
     * @param isEthDeposit A boolean indicating if the deposit amount in creating treasure box is in ETH (true) or tokens (false).
     */
...
// 1720581983,  [["0xB9e2A2008d3A58adD8CC1cE9c15BF6D4bB9C6d72", 1,2,0],["0xB9e2A2008d3A58adD8CC1cE9c15BF6D4bB9C6d72", 2,4,0],["0xB9e2A2008d3A58adD8CC1cE9c15BF6D4bB9C6d72", 3,6,0]]
// 1720581983